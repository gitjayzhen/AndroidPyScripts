...
from android_testrunner import adb_interface,am_instrument_parser,errors
...
class AndroidCommands(object):
  ...
  def __init__(self, device=None, api_strict_mode=True):
    """Constructor.
  ...
  def RunShellCommand(self, command, timeout_time=20, log_result=False):
    ...
    # print logging.warning(command + " contains ' quotes")
    if "'" in command:
        logging.warning(command + " contains ' quotes")
    result = self._adb.SendShellCommand(command, timeout_time).splitlines()
