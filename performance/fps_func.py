...
class SurfaceFlinger(object):
    ...
    def __clear_buffer(self):
        results = subprocess.Popen("adb shell {0} {1}".format(self.CLEAR_BUFFER_CMD, self.activity_name), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.readlines()
    ...
    def start_dump_latency_data(self, ignore_pending_fence_time = False):
        ...
        if self.__clear_buffer():
            ...
            results = subprocess.Popen("adb shell {0} {1}".format(self.FRAME_LATENCY_CMD, self.activity_name), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.readlines()
...
def write_csv(*list):
    ...
    if not os.path.isdir(path):
        os.makedirs(path)
    f = open(PATH("%s/fps-%s.csv" % (path, time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time())))), "w")
    times = list[0]
    fps = list[1]
    jankniess = list[2]
...
if __name__ == "__main__":
    if not raw_input("Make sure the test Activity, in this process, you should keep in this Activity!\n"
        ...
        activity_name = subprocess.Popen("adb shell dumpsys activity activities | findstr mFocusedActivity", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.read().split()[-1][:-1]
